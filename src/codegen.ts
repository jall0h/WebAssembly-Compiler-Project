...
(import "functions" "read" (func $read (result i32)))\n(import "js" "mem" (memory 1))`
    ...
    visitDefn(ctx: DefnContext, ts: type_env): [string ,type_env] {
        ...
        if (ctx.DEF()  && ctx.L_PAREN() && ctx.ID() && ctx.arg_list() && ctx.COLON() && ctx.EQUAL() && ctx.exp(0)){
            if (ctx.STRING_TYPE()){
                const new_ts: type_env = new Map([...ts, [ctx.ID().getText(), ["String", ...this.getArgumentTypes(ctx.arg_list(),ts)]]])
            ...
            if (ctx.INT() && ctx.L_SQUARE_PAREN() && ctx.R_SQUARE_PAREN()){
                const new_ts: type_env = new Map([...ts, [ctx.ID().getText(), ["Int[]", ...this.getArgumentTypes(ctx.arg_list(),ts)]]])
                const [params, u_ts] = this.createArguments(ctx.arg_list(),new_ts)
            ...
            if (ctx.DOUBLE() && ctx.L_SQUARE_PAREN() && ctx.R_SQUARE_PAREN()){
                const new_ts: type_env = new Map([...ts, [ctx.ID().getText(), ["Double[]", ...this.getArgumentTypes(ctx.arg_list(),ts)]]])
                const [params, u_ts] = this.createArguments(ctx.arg_list(),new_ts)
            ...
            if (ctx.VOID()){
                const new_ts: type_env = new Map([...ts, [ctx.ID().getText(), ["Void", ...this.getArgumentTypes(ctx.arg_list(),ts)]]])
                const [params, u_ts] = this.createArguments(ctx.arg_list(),new_ts)
